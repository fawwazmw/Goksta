...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (empty($title) || empty($content)) {
    header("Location:data-masters-tags-add.php?notif=tambahkosong&jenis=fieldkosong");
    exit();
  }
  ...
  // Construct SQL Query
  $sql = "INSERT INTO tags (parent_id, title, meta_title, slug, content) VALUES (?, ?, ?, ?, ?)";

  // Prepare and bind
  if ($stmt = mysqli_prepare($conn, $sql)) {
    // Bind parameters
    if ($parent_id === NULL || !is_numeric($parent_id)) {
      $parent_id = NULL; // Ensure it's NULL if not numeric or not set
    }
    ...
    if (mysqli_stmt_execute($stmt)) {
      if ($action === 'publish') {
        header("Location:data-masters-tags.php?notif=tambahberhasil");
      } else {
