...
<header class="app-header flex items-center px-4 gap-3">
    ...
    <div class="relative">
        <button data-fc-type="dropdown" data-fc-placement="bottom-end" type="button" class="nav-link">
            <?php
            // Include file koneksi database dan session
            include('services/database.php'); // Menghubungkan ke file koneksi database
            include_once('services/session.php'); // Menghubungkan ke file session.php

            // Pastikan pengguna telah login sebelum menampilkan gambar profil
            if (is_user_logged_in()) {
                $user_id = $_SESSION['id_user']; // Mendapatkan id_user dari sesi

                // Query untuk mengambil data profil dari tabel users
                $sql = "SELECT profile FROM users WHERE id_user = $user_id";
                $result = mysqli_query($conn, $sql);

                if ($result && mysqli_num_rows($result) > 0) {
                    $user_data = mysqli_fetch_assoc($result);
                    $profile_image_from_db = $user_data['profile'];

                    // Pastikan profile tidak kosong dan memiliki nilai yang sesuai
                    if (!empty($profile_image_from_db)) {
                        // Menghilangkan 7 karakter pertama dari nilai profile jika perlu
                        $profile_image = substr($profile_image_from_db, 7);

                        // Menampilkan tag <img> dengan atribut src yang sudah dimodifikasi
                        echo '<img src="' . htmlspecialchars($profile_image) . '" alt="user-image" class="rounded-full h-10">';
                    } else {
                        // Jika profile kosong, tampilkan gambar default
                        echo '<img src="assets/images/users/adam.png" alt="user-image" class="rounded-full h-10">';
                    }
                } else {
                    // Jika query gagal atau tidak ada hasil, tampilkan gambar default
                    echo '<img src="assets/images/users/adam.png" alt="user-image" class="rounded-full h-10">';
                }
            } else {
                // Jika tidak ada yang login, tampilkan default atau tautan login
                echo '<img src="assets/images/users/adam.png" alt="user-image" class="rounded-full h-10">';
            }
            ?>
        </button>
        <div class="fc-dropdown fc-dropdown-open:opacity-100 hidden opacity-0 w-44 z-50 transition-[margin,opacity] duration-300 mt-2 bg-white shadow-lg border rounded-lg p-2 border-gray-200 dark:border-gray-700 dark:bg-gray-800">
            ...
            <a class="flex items-center py-2 px-3 rounded-md text-sm text-gray-800 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-gray-300" href="auth-logout.php">
